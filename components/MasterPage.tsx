...
const MasterPage = () => {
  ...
  const data = capMasterTextByLang[lang as Language][0];
  ...
  const mid = Math.floor(master.length / 2);
  const firstRow = master.slice(0, mid);
  const secondRow = master.slice(mid);
